...
drop function if exists api.fn_resumen_planes_seguimiento(integer,integer);
create function api.fn_resumen_planes_seguimiento(p_empresa integer,p_app integer)
returns table(f_total bigint,f_contactar_hoy bigint,f_atrasados bigint,f_proximos_3_dias bigint,f_completados bigint)
language sql stable security definer set search_path=api,public as $$
  with parametros as (
    select (current_timestamp at time zone 'America/Santo_Domingo')::date as hoy
  )
  select count(*),
    count(*) filter(where f_estado='active' and f_fecha_contacto=x.hoy),
    count(*) filter(where f_estado='active' and f_fecha_contacto<x.hoy),
    count(*) filter(where f_estado='active' and f_fecha_contacto>x.hoy and f_fecha_contacto<=x.hoy+interval '3 days'),
    count(*) filter(where f_estado='completed')
  from api.t_planes_seguimiento cross join parametros x
