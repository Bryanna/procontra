...
const decimalDigits = (value: string) => { const normalized = value.replace(/[^0-9.]/g, ""); const [whole, ...decimals] = normalized.split("."); return decimals.length ? `${whole}.${decimals.join("")}` : whole; };
const integerDigits = (value: string) => value.replace(/\D/g, "");
...
export function NewPlanWorkspace({
  ...
  const localMatches = useMemo(() => query ? patients.filter((patient) => normalize(`${patient.code} ${patient.name} ${patient.phone} ${patient.insurer ?? ""} ${patient.insuranceCardMask ?? ""} ${patient.branch ?? ""} ${patient.branchCode ?? ""}`).includes(query)).slice(0, 8) : [], [patients, query]);
  ...
  const prescriptionNumberPreview = `RECETA-${todayIso().slice(0, 4)}-######`;
