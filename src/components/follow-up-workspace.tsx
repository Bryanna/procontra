...
function ResultDialog({ plan, canDispense, onClose }: { plan: FollowUpPlan; canDispense: boolean; onClose: () => void }) {
  ...
  return <div className="followup-modal" role="dialog" aria-modal="true" aria-label={`Registrar resultado de ${plan.patientName}`}>
    ...
    <section className="followup-modal-card followup-result-card"><header className="followup-modal-header"><div className="followup-modal-title"><span className="followup-modal-icon"><Phone size={21} /></span><div><span className="eyebrow">REGISTRAR CONTACTO</span><h2>{plan.patientName}</h2><p>{formatPhoneNumber(plan.phone)} · {plan.insurer}</p></div></div><button className="followup-icon-btn" type="button" aria-label="Cerrar" onClick={onClose}><X size={20} /></button></header>
...
function TimelineDialog({ plan, onClose }: { plan: FollowUpPlan; onClose: () => void }) {
  ...
  return <div className="followup-modal" role="dialog" aria-modal="true" aria-label={`Trazabilidad de ${plan.patientName}`}>
    ...
    <section className="followup-modal-card followup-timeline-card">
      <header className="followup-modal-header"><div className="followup-modal-title"><span className="followup-modal-icon"><History size={21}/></span><div><span className="eyebrow">TRAZABILIDAD DEL SEGUIMIENTO</span><h2>{plan.patientName}</h2><p>{formatPhoneNumber(plan.phone)} · {plan.insurer}</p></div></div><button className="followup-icon-btn" type="button" aria-label="Cerrar" onClick={onClose}><X size={20}/></button></header>
